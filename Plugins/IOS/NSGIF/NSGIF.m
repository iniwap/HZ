...
+ (void)optimalGIFfromURL:(NSURL*)videoURL loopCount:(int)loopCount quility:(int)quility completion:(void(^)(NSURL *GifURL))completionBlock {
    ...
    float videoLength = (float)asset.duration.value/asset.duration.timescale/2.0f;
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        gifURL = [self createGIFforTimePoints:timePoints fromURL:videoURL fileProperties:fileProperties frameProperties:frameProperties frameCount:frameCount gifSize:optimalSize quility:quility];
...
+ (void)createGIFfromURL:(NSURL*)videoURL withFrameCount:(int)frameCount delayTime:(float)delayTime loopCount:(int)loopCount quility:(int)quility completion:(void(^)(NSURL *GifURL))completionBlock {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        gifURL = [self createGIFforTimePoints:timePoints fromURL:videoURL fileProperties:fileProperties frameProperties:frameProperties frameCount:frameCount gifSize:GIFSizeMedium quility:quility];
...
+ (NSURL *)createGIFforTimePoints:(NSArray *)timePoints fromURL:(NSURL *)url fileProperties:(NSDictionary *)fileProperties frameProperties:(NSDictionary *)frameProperties frameCount:(int)frameCount gifSize:(GIFSize)gifSize quility:(int)quility{
    ...
    for (NSValue *time in timePoints) {
        CGImageRef imageRef;
            ...
            imageRef = (gifSize != GIFSizeOriginal && quility != 0 )? createImageWithScale([generator copyCGImageAtTime:[time CMTimeValue] actualTime:nil error:&error], (float)gifSize/10/quility) : [generator copyCGImageAtTime:[time CMTimeValue] actualTime:nil error:&error];
        #elif TARGET_OS_MAC
